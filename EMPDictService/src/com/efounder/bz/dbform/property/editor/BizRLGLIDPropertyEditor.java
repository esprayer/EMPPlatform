 ...
 * @version 1.0
 */
public class BizRLGLIDPropertyEditor extends AbstractPropertyEditor implements ActionListener{
...
//    if ( dataSetComponent == null ) return;
    String[] dataSetKeys = dctMetaData.getRLGL_IDs();//dataSetComponent.getDataSetKey();
    ...
    for(int i=0;i<dataSetKeys.length;i++) {
      ...
      DCTMetaData mdm= dctMetaData.getRLGLDCTMetaData(dataSetKey);
       EFRowSet ers=EFRowSet.getInstance();
       ers.putString("RLGL_ID",dataSetKey);
       ers.putString("RLGL_MC",dataSetKey);
       if(mdm!=null)  ers.putString("RLGL_MC",mdm.getDCT_MC());
       dctEds.insertRowSet(ers);
     }
   }
   EFDataSet dctEds;

  public BizRLGLIDPropertyEditor() {
  ...
  public void actionPerformed(ActionEvent e) {
    if(e.getSource()==bthelp){
     ...
     HelpInfoContext hic = new HelpInfoContext();
       ...
       hic.setHelpColumn(new String[] {"RLGL_ID", "RLGL_MC"});
       hic.setBHColumn("RLGL_ID");
       hic.setMCColumn("RLGL_MC");
...
//       if (so != null) {
//           mess[0] = so.getString("RLGL_ID", "");
//           mess[1] = so.getString("RLGL_MC", "");
//           Object oldFont = dataObject;
//           dataObject =  mess[0];
//           firePropertyChange(oldFont, dataObject);
//       }
