...
public class ProxyLookup extends Lookup {
    ...
    */
     ...
     * @since 1.31
     */
    protected void beforeLookup (Template template,Object key) {
    }
    /**
     *
     * @param clazz Class
     ...
     * @return Object
     */
    protected Object lookupMulInstance(Class clazz,Object key) {
      Object instance = null;
      java.util.Vector clazzList = PackageStub.getContentVector(clazz.getName());
      if ( clazzList == null ) return null;
      Enumeration en = clazzList.elements();
      while ( en.hasMoreElements() ) {
        StubObject SO = (StubObject) en.nextElement();
        try {
          String line = SO.getString("implement", null); //reader.readLine();
          // ������Ϊ�գ�������һ��
          ...
          line = line.trim();
          Class cla = this.getClass().getClassLoader().loadClass(line);
    ...
    protected java.util.Map lookupObjectMap = new HashMap();
    /**
     *
     * @param clazz Class
     * @param key Object
     * @return Object
     */
    protected Object lookupSingInstance(Class clazz,Object key) {
      Object instance = null;String objKey = null;
      objKey = clazz.getName()+(key==null?"":key.toString());
      Object inst = lookupObjectMap.get(objKey);
      ...
      while ( en.hasMoreElements() ) {
        ...
        try {
          ...
          Class cla = this.getClass().getClassLoader().loadClass(line);
          instance = cla.newInstance();
          lookupObjectMap.put(objKey,instance);
        }
        catch (Exception e) {
          e.printStackTrace();
        }
     ...
     *    has been found
     */
    public final Object lookup (Class clazz,Object key,boolean sign) {
      // ����Ƕ�ʵ��ʽ���򵥶�����
      if ( !sign ) {
        return lookupMulInstance(clazz,key);
      } else {
        return lookupSingInstance(clazz,key);
      }
//        beforeLookup (new Template (clazz),key);
//
//        Lookup[] lookups = this.lookups;
//
//        for (int i = 0; i < lookups.length; i++) {
//            Object o = lookups[i].lookup (clazz,key);
//            if (o != null) {
//                return o;
//            }
//        }
...
//        return null;
     ...
     * @return item or null
     */
    public final Item lookupItem(Template template,Object key) {
        beforeLookup (template,key);

        Lookup[] lookups = this.lookups;

        for (int i = 0; i < lookups.length; i++) {
            Item o = lookups[i].lookupItem (template,key);
            if (o != null) {
                return o;
            }
        }
     ...
     */
    public final synchronized Result lookup (Lookup.Template template,Object key) {
        R r;
        if (results != null) {
...
//                }
        ...
        private Lookup.Result[] myBeforeLookup () {
            ProxyLookup.this.beforeLookup (template,null);
