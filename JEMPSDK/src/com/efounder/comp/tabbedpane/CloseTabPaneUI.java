 ...
 * @author Dave Moore
 */
public class CloseTabPaneUI extends AlloyTabbedPaneUI {
...
//	private BufferedImage closeImgINoSelect;

//	private BufferedImage maxImgINoSelect;
...
//	protected JPopupMenu actionPopupMenu;
	...
	protected JMenuItem closeItem;
        private static final Color selectedColor = SystemColor.textHighlight;//new Color(15, 70, 180);
	public CloseTabPaneUI() {
		...
		maxImgI = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
				BufferedImage.TYPE_4BYTE_ABGR);
//    closeImgINoSelect = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
//                    BufferedImage.TYPE_4BYTE_ABGR);

//    maxImgINoSelect = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
//                    BufferedImage.TYPE_4BYTE_ABGR);
...
//                                closeImgINoSelect.createGraphics(), 0, 0);
//                closeB.setBackground(selectedColor);
		getResIcon().paintIcon(closeB,
				closeImgI.createGraphics(), 0, 0);

                // 最大化图标
//                WindowsIconFactory.createFrameMaximizeIcon().paintIcon(maxB,
//                                maxImgINoSelect.createGraphics(), 0, 0);
//                maxB.setBackground(selectedColor);
		getMaxIcon().paintIcon(maxB,
...
//		setPopupMenu();
        ...
        protected Icon getMaxIcon() {
          Icon maxIcon = (Icon)System.getProperties().get("MAX_ICON");
          if ( maxIcon == null )
            maxIcon = WindowsIconFactory.createFrameMaximizeIcon();
          return maxIcon;
        }
        protected Icon getResIcon() {
          Icon resIcon = (Icon)System.getProperties().get("RES_ICON");
          if ( resIcon == null )
            resIcon = WindowsIconFactory.createFrameCloseIcon();
          return resIcon;
        }
...
//			actionPopupMenu.add(closeItem);
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
		...
		String title = tabPane.getTitleAt(tabIndex);
                String tooltips = tabPane.getToolTipTextAt(tabIndex);
                tabPane.setToolTipText(tooltips);
		...
		if (cropShape) {
			...
			if (isCloseButtonEnabled)
				paintCloseIcon(g2, dx, dy, isOver,isSelected);
			if (isMaxButtonEnabled)
				paintMaxIcon(g2, dx, dy, isOver,isSelected);
		}
	...
	protected void paintCloseIcon(Graphics g, int dx, int dy, boolean isOver,boolean isSelected) {
          ...
          bi = closeImgI;//isSelected?closeImgI:closeImgINoSelect;
		paintActionButton(g, dx, dy, closeIndexStatus, isOver, isSelected, closeB,
				closeImgB);
		g.drawImage(bi, dx, dy + 1, null);
	}

	protected void paintMaxIcon(Graphics g, int dx, int dy, boolean isOver,boolean isSelected) {
          BufferedImage bi = null;
          bi = maxImgI;//isSelected?maxImgI:maxImgINoSelect;
		if (isCloseButtonEnabled)
			dx -= BUTTONSIZE;
		paintActionButton(g, dx, dy, maxIndexStatus, isOver, isSelected, maxB, maxImgB);
		g.drawImage(bi, dx, dy + 1, null);
	}

	protected void paintActionButton(Graphics g, int dx, int dy, int status,
			boolean isOver,boolean isSelect, JButton button, BufferedImage image) {
		...
		if (isOver) {
			...
			case PRESSED:
                ...
                if ( isSelect ) {
                  ...
                  clr = UIManager.getColor("Button.background");
                } else {
                  clr = UIManager.getColor("Button.background");
                }
//                if ( isOver && isSelect ) {
	...
	class MyMouseHandler extends MouseHandler {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (isOneActionButtonEnabled() && e.isPopupTrigger()) {
				...
				maxIndexStatus = INACTIVE; //right-clicking on icons firePopupMenuTabEvent
                                ((CloseAndMaxTabbedPane) tabPane).firePopupMenuTabEvent(e, overTabIndex);
