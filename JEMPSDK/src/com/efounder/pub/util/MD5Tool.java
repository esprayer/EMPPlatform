...
public class MD5Tool {
  ...
  // 下面这些S11-S44实际上是一个4*4的矩阵，这样写是方便修改
    ...
    public String getMD5ofStr(String inbuf) {
      try {
          byte[] pBYTE = inbuf.getBytes("GB2312");
          md5Init();
          md5Update(pBYTE, pBYTE.length);
          md5Final();
          digestHexStr = "";
          for (int i = 0; i < 16; i++) {
              digestHexStr += byteHEX(digest[i]);
          }
          //做一点特殊处理 likebing 2008.5.4青年节
          digestHexStr = "PN" + digestHexStr;
          if (digestHexStr.length() > 30) {
              digestHexStr = digestHexStr.substring(0, 30);
          }
          return digestHexStr;
      } catch (Exception E) {
          E.printStackTrace();
    ...
    protected static MD5Tool md5 = null;
