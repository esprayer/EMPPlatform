...
import com.efounder.bz.dbform.component.dc.table.corner.*;
import com.efounder.bz.dbform.component.dc.table.footer.*;
import com.efounder.bz.dbform.component.dc.table.header.*;
import com.efounder.bz.dbform.component.dc.table.render.*;
 ...
 * @version 1.0
 */
public class DMTable extends JXTable implements FormComponent ,PropertyChangeListener,DCTDataSetContainer,DMComponent,DataSetListener,KeyListener, MouseListener,Scriptable,Readable, Runnable {
  /**
   *
   * @return TableModel 
   */
  protected static TableModel getDemoTableModel() {
    TableModel td = new DefaultTableModel(100,50);
    return td;
  }
  /**
   *
...
//add by fsz 
  ...
  public void valueChanged(ListSelectionEvent e) {
	  ...
	  if(getDCTableModel()==null||
		getDCTableModel().getDataSet()==null)return;
   ...
   */
  public DMTable(TableModel model) {
      super(model);
  }
  /**
   *
  ...
  public DCTableModel getDCTableModel() {
    ...
    if ( tableModel != null && tableModel instanceof DCTableModel )
      return (DCTableModel)tableModel;
    else
      return null;
  }
  /**
   ...
   */
  public void setDCTableModel(DCTableModel tableModel) {
    TableModel oldTM = this.getModel();
    if ( oldTM != null && oldTM instanceof DCTableModel ) {
         ((DCTableModel)oldTM).setListSelectionModel(null);
    }
    setModel(tableModel);
    if ( tableModel != null )
  ...
  /**
   *
   * @param newModel ListSelectionModel
   */
  public void setSelectionModel(ListSelectionModel newModel) {
    DCTableModel tableModel = this.getDCTableModel();
    if ( tableModel != null ) {
...
//      this.setAutoResizeMode(JTable.AUTO_RESIZE_SUBSEQUENT_COLUMNS);
  ...
  public void setGroupModel(ColumnHeaderGroupModel groupModel) {
    ...
    if (groupModel != old) {
      ...
      firePropertyChange("columnModel", this.getColModel(), this.getColModel());
      resizeAndRepaint();
    }
  }
...
//    if ( groupModel != null )groupModel.addColumnModelListener(this);
          ...
          if (this.getColumnFooter() != null) {
            if(groupModel.getFooterModel() instanceof FootCalcTableModel){
             ((FootCalcTableModel)groupModel.getFooterModel()).setDataSet(this.getDataSet());
            }
...
//            configureCorner(scrollPane, JScrollPane.UPPER_RIGHT_CORNER);
          }
          if ( this.columnFooterVisible && this.rowHeaderVisible ) {
            configureCorner(scrollPane, JScrollPane.LOWER_LEFT_CORNER);
//            configureCorner(scrollPane, JScrollPane.LOWER_RIGHT_CORNER);
          }
  ...
  /**
   * modify
   * @param scrollPane JideScrollPane
   */
...
//          scrollPane.getColumnFooter().setPreferredSize(new java.awt.Dimension(300,25));
//          scrollPane.setColumnFooter(viewport);
  ...
  /**
   *
   * @param rowHeader TableRowHeader
   */
  ...
  public TableGroupColumnFooter getColumnFooter() {
      if (columnFooter == null) {
          ...
          columnFooter.setDefaultRenderer(Object.class,
                                      new FooterTextRender(
                                              columnFooter));
  ...
  /**
   *
   * @param columnFooter TableColumnFooter
   */
   ...
   */
  protected JideScrollPane getJideScrollPane() {
    Container viewport = getParent();
    if (viewport instanceof JViewport) {
      Container parent = viewport.getParent();
      if (parent instanceof JideScrollPane) {
        return (JideScrollPane) parent;
      }
    }
    return null;
  }
  ...
  /**
   *
   * @param columnHeaderVisible boolean
   */
          ...
          if (viewport != null && ((JViewport) viewport).getView() == this) {
            if (columnHeaderVisible) {
              ...
              if ( this.rowHeaderVisible )
                this.configureCorner(scrollPane,JScrollPane.UPPER_LEFT_CORNER);
            }
            else {
              scrollPane.setColumnHeaderView(null);
  ...
  /**
   *
   * @param rowHeaderVisible boolean
   */
          ...
          if (viewport != null && ((JViewport) viewport).getView() == this) {
            ...
            if ( rowHeaderVisible ) {
              ...
              if ( this.columnFooterVisible )
                this.configureCorner(scrollPane,JScrollPane.LOWER_LEFT_CORNER);
            }
            else {
              scrollPane.setRowHeaderView(null);
              scrollPane.setCorner(JScrollPane.UPPER_LEFT_CORNER,null);
...
//              scrollPane.setColumnFooterView(this.getColumnFooter());
    ...
    firePropertyChange("columnFooterVisible", !columnFooterVisible, columnFooterVisible);
  }
...
//    setCellSelectionEnabled(true);
  ...
  public void propertyChange(PropertyChangeEvent e) {
    ...
    if ( this.tableHeader != null ) {
      this.tableHeader.validate();this.tableHeader.repaint();
    }
    if ( this.columnFooter != null ) {
      this.columnFooter.validate();this.columnFooter.repaint();
    }
  }
  ...
  public void setDataSet(EFDataSet ds) {
    if (dataSet != ds) {
      ...
      if(getGroupFooterModel()!=null&&getGroupFooterModel().getFooterModel() instanceof FootCalcTableModel){
        ((FootCalcTableModel)getGroupFooterModel().getFooterModel()).setDataSet(dataSet);
      }
    }
  }
  /**
  ...
  public void stopEditing() {
      ...
      if (editor != null) {
          ...
          //
          getModel().setValueAt(value, convertRowIndexToModel(editingRow),
...
//          setValueAt(value, editingRow, editingColumn);
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.getKeyCode() == KeyEvent.VK_ENTER ||
      ...
      while(coln>=0&&getColumnModel().getColumn(coln).getWidth()==0)coln--;
      boolean cedit=true;
      if(this.getModel() instanceof DCTableModel)
          cedit=((DCTableModel)getModel()).isEnableDataEdit();
      if(cedit&&isEditable()&&row==this.getRowCount()-1&&col==coln){
        DCTableModel model=this.getDCTableModel();
...
//    this.setRowSelectionInterval(rowIndex,rowIndex);
    ...
    if(rows!=null&&rows.length>1)rown=rows[rows.length-1];
  ...
  public void mouseClicked(MouseEvent e) {
      if (e.getClickCount() == 2 && e.getButton() == e.BUTTON1) {
          ...
          if (e.getSource() == getTableHeader() && isColumnSortable()) {
              ...
              WaitingManager.getDefault().waitInvoke(this,"提示", "正在按列["+sortColumn.getHeaderValue()+"]进行排序.......", null, this);
  ...
  protected void sort(Column column){
      ...
      try{
    	  for(int i=0;dsl!=null&&i<dsl.length;i++)//禁止前面刷新，要不排一条刷一条，难看
...
//  }
  ...
  public String getToolTipText(MouseEvent e) {
      ...
      try{
          ...
          if(tiptextString == null) tiptextString = this.getStringAt(rowIndex, colIndex);
  ...
  public String getCusToolTipText(int row,int col) throws Exception {
      return (String)runScript(this, "getToolTipText",
                     new String[] {"tableObj","row","col"},
                     new Object[] {this,row,col});

 }

 protected Object runScript(Scriptable script,String method,String[]mname,Object[] param){
      try {
          return ScriptUtil.runComponentFunction(script, method,mname,param);
      } catch ( Exception ex ) {
         ex.printStackTrace();
