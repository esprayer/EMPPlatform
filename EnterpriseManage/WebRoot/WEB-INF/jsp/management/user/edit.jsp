...
<form method="post" action="${ctx}/management/base/user/update/${userobj.USER_ID}?USER_ORGID=${userobj.USER_ORGID}&rel=userLiBox&callbackType=closeCurrent" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
	<div class="pageFormContent" layoutH="56">
		<p>
			<label>用户编号: </label>
			<input disabled="true" type="text" name="USER_ID" value="${userobj.USER_ID}" class="required"/>
		</p>
		<p>
			<label>用户名称: </label>
		...
		<p>
			...
			<select name="USER_DISABLE" style="width:135px" class="combox" >
				<option value=1 <c:if test="${userobj.USER_DISABLE==1}">selected="selected"</c:if>>启用</option>
				<option value=0 <c:if test="${userobj.USER_DISABLE==0}">selected="selected"</c:if>>停用</option>
			</select>		
		</p>	
	</div>
	<div class="formBar">
