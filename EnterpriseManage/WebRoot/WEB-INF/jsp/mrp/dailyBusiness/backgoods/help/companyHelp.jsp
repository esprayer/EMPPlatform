...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="${ctx}/dailyBusiness/backGoods/help/companyHelp" onsubmit="return dwzSearch(this, 'dialog');">
		<div class="searchBar" >
		...
		<tbody>
			<c:forEach var="item" items="${hycsList}" varStatus="s">
				<tr target="slt_objId" rel="${item.f_CSBH}">
					...
					<td>${item.f_CSMC}</td>				
					<td>
						<a class="btnSelect" href="javascript:$.bringBack({F_THRID:'', F_THRMC:'', F_CSMC:'${item.f_CSMC}', F_CSBH:'${item.f_CSBH}'})" title="查找带回">选择</a>
					</td>
				</tr>	
			</c:forEach>
		</tbody>
