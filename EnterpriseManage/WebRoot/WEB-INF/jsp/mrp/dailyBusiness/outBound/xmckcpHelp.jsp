...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="${ctx}/dictHelp/project/product/help?F_XMBH=${F_XMBH}&F_CKBH=${F_CKBH}&F_XMZT=0&HELPCOL=F_YYCPBH,F_YYCPMC&tarRel=/mrp/dailyBusiness/outBound/xmckcpHelp" onsubmit="return dwzSearch(this, 'dialog');">
	<div class="searchBar" >
		<ul class="searchContent">
			<li>
				<label>产品编号:</label>
				<input class="textInput" name="F_CPBH" value="${F_CPBH}" type="text">
			</li>	  		
		...
		<tbody>
			<c:forEach var="item" items="${productList}" varStatus="s">
				<tr target="slt_objId" rel="${item.f_CPBH}">
					...
					<td>${item.f_CPMC}</td>					
					<td>
						<a class="btnSelect" href="javascript:$.bringBack({${F_ZDMC1}:'${item.f_CPMC}', ${F_ZDBH1}:'${item.f_CPBH}', ${F_ZDMC2}:'${item.f_CPMC}', ${F_ZDBH2}:'${item.f_CPBH}'})" title="查找带回">选择</a>
					</td>
				</tr>	
			</c:forEach>
		</tbody>
