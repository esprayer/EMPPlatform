...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="${ctx}/dictHelp/project/help/search/${F_CLBH}?F_CRFX=R&F_XMZT=${F_XMZT}&tarRel=/dailyBusiness/outBound/xmHelp" onsubmit="return dwzSearch(this, 'dialog');">
		...
		<tbody>
			<c:forEach var="item" items="${projectList}" varStatus="s">
				<tr target="slt_objId" rel="${item.f_XMBH}">
					...
					<td>
						<a class="btnSelect" href="javascript:$.bringBack({F_YYXMMC:'${item.f_XMMC}', F_YYXMBH:'${item.f_XMBH}'})" title="查找带回">选择</a>
					</td>
				</tr>	
			</c:forEach>
		</tbody>
