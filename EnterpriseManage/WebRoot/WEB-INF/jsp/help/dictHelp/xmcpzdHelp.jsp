...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="${ctx}/dictHelp/product/help/search/${F_XMBH}?F_ZDBH=${F_ZDBH}&F_ZDMC=${F_ZDMC}" onsubmit="return dwzSearch(this, 'dialog');">
	<div class="searchBar" >
		...
		<tbody>
			<c:forEach var="item" items="${productList}" varStatus="s">
				<tr target="slt_objId" rel="${item.f_CPBH}">
					...
					<td>${item.f_CPMC}</td>					
					<td>
						<a class="btnSelect" href="javascript:$.bringBack({${F_ZDMC}:'${item.f_CPMC}', ${F_ZDBH}:'${item.f_CPBH}'})" title="查找带回">选择</a>
					</td>
				</tr>	
			</c:forEach>
		</tbody>
