 ...
 */
@Repository
public class EMPProjectDetailsQuery {
	...
	private void createTempTable(Statement stmt) throws Exception{
	    msTableName = "XMCX"+StringFunction.getTempStr(6);
	    String        strSql = "";
	    ...
	    String tempTableBody = " F_ID          varchar(1)           default ''   null,"
	         				 ...
	         				 + " F_XMJDCB      decimal(30,2)   	    default 0    null,"
	         				 + " F_XMBJDCB     decimal(30,2)   	    default 0    null,"
	...
	public void getOriginData(Statement stmt, JParamObject po) throws Exception {
		...
		//插入借调出库数量,同一个项目不同产品之间的调货,影响库存成本和领用成本，对于项目口径查询，认为是正常出库
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH, F_CPBH, F_CLBH, F_CRFX, F_FLLX, F_CLDJ, F_ZCCB)"
		       + " select '0', F_YYXMBH, F_YYCPBH, F_CLBH, 'C', '0', F_CLDJ, round(sum(F_CLSL * F_CLDJ), 2)"
		       + " from HYXMCK"
		       + " where HYXMCK.F_CRFX = 'C' and F_FLLX = '1' and HYXMCK.F_CLBH like '" + F_CLBH + "%'"		       
		       + " and HYXMCK.F_YYCPBH like '" + F_CPBH + "%'"
		       + " and HYXMCK.F_CKBH like '" + F_CKBH + "%'"
		       + " and HYXMCK.F_YYXMBH like '" + F_XMBH + "%' AND HYXMCK.F_XMBH = HYXMCK.F_YYXMBH"
		       + " and exists(select 1 from HYXMZD where (HYXMZD.F_SQSJ >= '" + beginDate + "' AND HYXMZD.F_SQSJ <= '" + endDate + "')"
		       + " and HYXMZD.F_XMBH = HYXMCK.F_YYXMBH)"
		       + " group by F_YYXMBH, F_YYCPBH, F_CLBH, F_CLDJ";
		stmt.execute(strSql);
		
		//插入借调出库数量,从别的项目借调,不同项目之间的调货，不影响库存成本，只影响领用成本
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH, F_CPBH, F_CLBH, F_CRFX, F_FLLX, F_CLDJ, F_XMJDCB)"
		...
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH, F_CPBH, F_CLBH, F_CLDJ, F_XMBJDCB)"
		       + " select '0', F_XMBH, F_CPBH, F_CLBH, F_CLDJ, round(sum(F_CLSL * F_CLDJ), 2)"
		       + " from HYXMCK"
		       + " where HYXMCK.F_CRFX = 'C' and F_FLLX = '1' and HYXMCK.F_CLBH like '" + F_CLBH + "%'"		       
		       + " and HYXMCK.F_CPBH like '" + F_CPBH + "%'"
		       + " and HYXMCK.F_CKBH like '" + F_CKBH + "%'"
		       + " and HYXMCK.F_XMBH like '" + F_XMBH + "%' AND HYXMCK.F_XMBH <> HYXMCK.F_YYXMBH"//所属项目编号不能等于应用项目编号，否则会虚增一块出库数量，相同项目不同产品之间的调货，项目出库数量不变化
		       + " and exists(select 1 from HYXMZD where (HYXMZD.F_SQSJ >= '" + beginDate + "' AND HYXMZD.F_SQSJ <= '" + endDate + "')"
		       + " and HYXMZD.F_XMBH = HYXMCK.F_XMBH)"
		       + " group by F_XMBH, F_CPBH, F_CLBH, F_CLDJ";
		stmt.execute(strSql);
	...
	public EFDataSet convertDataSet(Statement stmt, JParamObject po) throws Exception {
		...
		//汇总数据
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH, F_RKCB, F_ZCCB, F_XMJDCB, F_XMBJDCB, F_BJDCB, F_CLTHCB, F_CSTHCB, F_DBCB)"
		       + " select '1', F_XMBH, round(sum(F_RKCB), 2), round(sum(F_ZCCB), 2), round(sum(F_XMJDCB), 2), round(sum(F_XMBJDCB), 2),"
		       + " round(sum(F_BJDCB), 2), round(sum(F_CLTHCB), 2), round(sum(F_CSTHCB), 2), round(sum(F_DBCB), 2)"
		       + " from " + msTableName
		       + " where F_ID = '0' "		       
		       + " group by F_XMBH";
		stmt.execute(strSql);
		...
		/**
		 * 以上费用成本包括
		 * 1.入库材料成本(F_RKCB)；2.正常出库材料成本(F_ZCCB)；3.借给其他项目材料成本(F_XMBJDCB)；
		 * 4.从别的项目借调材料成本(F_XMJDCB)；5.仓库调拨材料成本(F_DBCB)；6.材料退货成本(F_CLTHCB)；7.厂商退货成本(F_CSTHCB)
		 */
		//更新库存成本
		//库存成本=入库成本-正常出库成本-借给其他项目成本+材料退货成本
		strSql = " update " + msTableName + " TEMP"
	           + " set F_KCCB = F_RKCB - F_ZCCB - F_XMBJDCB + F_CLTHCB" ;
		stmt.execute(strSql);
		
		//更新项目成本
		//项目成本=正常出库成本+从别的项目借调成本+仓库调拨成本-材料退货成本-厂商退货成本
		strSql = " update " + msTableName + " TEMP"
	           + " set F_XMCB = F_ZCCB + F_XMJDCB + F_DBCB  - F_CLTHCB - F_CSTHCB" ;
		stmt.execute(strSql);
		...
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH)"
	       	   + " select '1', F_XMBH"
	       	   + " from HYXMZD"
	       	   + " where HYXMZD.F_SQSJ >= '" + beginDate + "' AND HYXMZD.F_SQSJ <= '" + endDate + "'"
	       	   + " and F_XMBH like '" + po.GetValueByParamName("F_XMBH", "") + "%'";	       
		stmt.execute(strSql);
		
		strSql = " insert into " + msTableName + " (F_ID, F_XMBH, F_KCCB, F_XMCB, F_ZCB)"
	       	   + " select '2', F_XMBH, round(sum(F_KCCB), 2), round(sum(F_XMCB), 2), round(sum(F_ZCB), 2)"
	       	   + " from " + msTableName
	       	   + " where F_ID = '1' "		       
	       	   + " group by F_XMBH";
		stmt.execute(strSql);
		...
		strSql = " select F_XMBH, F_XMMC, F_XMZT, round(F_KCCB, 2) as F_KCCB, round(F_XMCB, 2) as F_XMCB, round(F_ZCB, 2) as F_ZCB from " + msTableName + " order by F_ID, F_XMBH";
