...
public class MultiSpanCellTable extends JTable {
  ...
  public MultiSpanCellTable(TableColumnModel model){
  ...
  public Rectangle getCellRect(int row, int column, boolean includeSpacing) {
    ...
    cellFrame.y = row * (aCellHeight-1) ;
    cellFrame.height = n[CellSpan.ROW] * (aCellHeight-1);

    Enumeration enumeration = getColumnModel().getColumns();
    while (enumeration.hasMoreElements()) {
      TableColumn aColumn = (TableColumn)enumeration.nextElement();
      cellFrame.width = aColumn.getWidth()+ columnMargin/2;
